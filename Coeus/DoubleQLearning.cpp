...
DoubleQLearning::DoubleQLearning(NeuralNetwork* p_network_a, GradientAlgorithm* p_gradient_algorithm_a, NeuralNetwork* p_network_b, GradientAlgorithm* p_gradient_algorithm_b, const float p_gamma) {
...
float DoubleQLearning::train(Tensor* p_state0, const int p_action0, Tensor* p_state1, const float p_reward) {
	...
	if (RandomGenerator::get_instance().random() > 0.5) {
		...
		const float maxQs1a = calc_max_qa(p_state1, _network_b);
...
float DoubleQLearning::calc_max_qa(Tensor* p_state, NeuralNetwork* p_network) const {
