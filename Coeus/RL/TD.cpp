...
TD::TD(NeuralNetwork* p_network, const GRADIENT_RULE p_grad_rule, const float p_alpha, const float p_gamma) : ICritic(p_network, p_grad_rule, p_alpha),
	_gamma(p_gamma)
{
}
...
float TD::train(Tensor* p_state0, Tensor* p_action, Tensor* p_state1, const float p_reward, const bool p_final)
{
	...
	const float delta = p_final ? p_reward - Vs0 : p_reward + _gamma * Vs1 - Vs0;
